...
export function MainDashboardClient({ initialDataRows, initialProjectRows }: MainDashboardClientProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-2.5 sm:gap-3 p-2 sm:p-3 max-w-[1600px] mx-auto font-sans text-slate-800 antialiased pb-8">
      ...
      {/* ========================================================================= */}
      {/* 3. COST DISTRIBUTION (Clean & Airy)                                       */}
      {/* ========================================================================= */}
      <section className="bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200/80 shadow-2xs space-y-2.5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-slate-100 text-emerald-800 flex items-center justify-center shrink-0">
              <PieChart className="w-3.5 h-3.5" />
            </div>
            <span className="text-xs sm:text-sm font-bold text-slate-800">
              สัดส่วนต้นทุนตามหมวดสินค้า
            </span>
            <span className="text-xs text-slate-400">
              ({costBreakdown.length} หมวด)
            </span>
          </div>

          {/* Toggle Paid vs Pending */}
          <div className="flex items-center gap-2 text-xs">
            <div className="inline-flex bg-slate-100 p-0.5 rounded-lg border border-slate-200/60">
              ...
              >
                จ่ายแล้วจริง (฿{money(summary.cashPaid)})
              </button>
              {summary.pendingAP > 0 && (
                <button
                  type="button"
        ...
        {/* Minimalist Multi-Segment Progress Strip */}
        <div className="w-full h-2 rounded-full overflow-hidden flex bg-slate-100 p-0.5 gap-0.5">
          {costBreakdown.length > 0 ? (
            costBreakdown.map((item, idx) => (
              <div
                key={idx}
                className={`h-full rounded-full ${item.color} transition-all duration-300 hover:opacity-90`}
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap pt-0.5 text-xs">
          {costBreakdown.map((item, idx) => {
            const pendingAmt = Number((item as any).pendingAmount || 0);
            return (
              <div
                key={idx}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50/90 border border-slate-200/80 hover:bg-slate-100 transition"
              >
                ...
                {pendingAmt > 0 && (
                  <span className="text-amber-700 font-medium text-[11px] bg-amber-50 px-1 rounded border border-amber-200">
                    รอเบิก ฿{money(pendingAmt)}
                  </span>
                )}
              </div>
            );
        ...
        <div className="p-2.5 sm:p-3">
          ...
          {activeTab === "vat" && (
            <div className="space-y-2.5">
              ...
              {/* View Mode 1: Clean Cards View */}
              {viewMode === "cards" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-2.5">
                  
                  {/* Card 1: ค่าแรงบริษัท */}
                  {(() => {
                    const paidNet = summary.main3Paid.laborBeforeVat;
                    const pendingNet = summary.main3Pending.laborBeforeVat;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-blue-50 border border-blue-200/60 text-blue-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <Building2 className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ค่าแรงบริษัท</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-blue-50 text-blue-700 border border-blue-200/70 shrink-0">หัก 3%</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                                  รอเบิก ฿{money(pendingNet)}
                                </span>
                              ) : (
                                <span className="text-slate-400">{hasData ? "บันทึกแล้ว" : "ไม่มีรายการ"}</span>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main3Paid.laborBeforeVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">ภาษี 3%</div>
                            <div className="text-xs sm:text-sm font-semibold text-emerald-700">฿{money(summary.main3Paid.laborVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                            <div className="text-xs sm:text-sm font-extrabold text-indigo-950">฿{money(paidNet)}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(() => {
                    const paidNet = summary.main3Paid.materialBeforeVat + summary.main3Paid.materialVat;
                    const pendingNet = summary.main3Pending.materialBeforeVat + summary.main3Pending.materialVat;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-emerald-50 border border-emerald-200/60 text-emerald-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <Hammer className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ค่าของ (มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200/70 shrink-0">VAT 7%</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main3Paid.materialBeforeVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs sm:text-sm font-semibold text-emerald-700">฿{money(summary.main3Paid.materialVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                  ...
                  {(() => {
                    const paidNet = summary.main3Paid.fuelBeforeVat + summary.main3Paid.fuelVat;
                    const pendingNet = summary.main3Pending.fuelBeforeVat + summary.main3Pending.fuelVat;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-amber-50 border border-amber-200/60 text-amber-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <Fuel className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">น้ำมัน (มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-amber-50 text-amber-700 border border-amber-200/70 shrink-0">VAT 7%</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main3Paid.fuelBeforeVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs sm:text-sm font-semibold text-emerald-700">฿{money(summary.main3Paid.fuelVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                  ...
                  {(() => {
                    const paidNet = summary.main3Paid.repairBeforeVat + summary.main3Paid.repairVat;
                    const pendingNet = summary.main3Pending.repairBeforeVat + summary.main3Pending.repairVat;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-rose-50 border border-rose-200/60 text-rose-600 flex items-center justify-center shrink-0 shadow-2xs">
                            <Truck className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ซ่อมรถ (มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-rose-50 text-rose-700 border border-rose-200/70 shrink-0">VAT 7%</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main3Paid.repairBeforeVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs sm:text-sm font-semibold text-emerald-700">฿{money(summary.main3Paid.repairVat)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-2xs">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-[#0b3531] text-white font-bold border-b border-[#062e2b] text-[11px] uppercase tracking-wider">
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a]">รายการ</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">ก่อน VAT (จ่ายจริง)</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">คำนวณ VAT / ภาษี (บาท)</th>
                        <th className="py-2.5 px-3 text-right">ยอดรวมสุทธิ (โอนจริง)</th>
                      </tr>
                    </thead>
            ...
            <div className="space-y-2.5">
              ...
              {viewMode === "cards" ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 sm:gap-2.5">
                  ...
                  {/* Card 1: ค่าแรงบุคคล */}
                  {(() => {
                    const paidNet = summary.main4LaborPaid;
                    const pendingNet = summary.main4LaborPending;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-indigo-50 border border-indigo-200/60 text-indigo-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <Users className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ค่าแรงบุคคล</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200/70 shrink-0">หมวดแรงงาน</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">จ่ายจริง</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main4Paid.naturalLabor)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-amber-700 leading-none mb-0.5">หัก ณ ที่จ่าย</div>
                            <div className="text-xs sm:text-sm font-semibold text-amber-700">฿{money(summary.main4Paid.naturalLaborDeduct)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-emerald-800 leading-none mb-0.5">โอนจริง</div>
                            <div className="text-xs sm:text-sm font-extrabold text-emerald-700">฿{money(summary.main4Paid.naturalLabor - summary.main4Paid.naturalLaborDeduct)}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {/* Card 2: พนักงาน */}
                  {(() => {
                    const paidNet = summary.main4StaffPaid;
                    const pendingNet = summary.main4StaffPending;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-sky-50 border border-sky-200/60 text-sky-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <UserCheck className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">พนักงาน</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-sky-50 text-sky-700 border border-sky-200/70 shrink-0">เงินเดือน/เบี้ยเลี้ยง</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">จ่ายจริง</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main4Paid.staff)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-amber-700 leading-none mb-0.5">หัก ณ ที่จ่าย</div>
                            <div className="text-xs sm:text-sm font-semibold text-amber-700">฿{money(summary.main4Paid.staffDeduct)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-emerald-800 leading-none mb-0.5">โอนจริง</div>
                            <div className="text-xs sm:text-sm font-extrabold text-emerald-700">฿{money(summary.main4Paid.staff - summary.main4Paid.staffDeduct)}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(() => {
                    const paidNet = summary.main4MaterialPaid;
                    const pendingNet = summary.main4MaterialPending;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ค่าของ (ไม่มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-slate-100 text-slate-700 border border-slate-200/70 shrink-0">วัสดุอุปกรณ์</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">จ่ายจริง</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main4Paid.material)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-amber-700 leading-none mb-0.5">หัก ณ ที่จ่าย</div>
                            <div className="text-xs sm:text-sm font-semibold text-amber-700">฿{money(summary.main4Paid.materialDeduct)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-emerald-800 leading-none mb-0.5">โอนจริง</div>
                            <div className="text-xs sm:text-sm font-extrabold text-emerald-700">฿{money(summary.main4Paid.material - summary.main4Paid.materialDeduct)}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(() => {
                    const paidNet = summary.main4FuelPaid;
                    const pendingNet = summary.main4FuelPending;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">น้ำมัน (ไม่มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-slate-100 text-slate-700 border border-slate-200/70 shrink-0">เชื้อเพลิง</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">จ่ายจริง</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main4Paid.fuel)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-amber-700 leading-none mb-0.5">หัก ณ ที่จ่าย</div>
                            <div className="text-xs sm:text-sm font-semibold text-amber-700">฿{money(summary.main4Paid.fuelDeduct)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-emerald-800 leading-none mb-0.5">โอนจริง</div>
                            <div className="text-xs sm:text-sm font-extrabold text-emerald-700">฿{money(summary.main4Paid.fuel - summary.main4Paid.fuelDeduct)}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(() => {
                    const paidNet = summary.main4RepairPaid;
                    const pendingNet = summary.main4RepairPending;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-900 truncate">ซ่อมรถ (ไม่มี VAT)</span>
                              <span className="text-[10px] font-semibold px-1.5 py-0.2 rounded-md bg-slate-100 text-slate-700 border border-slate-200/70 shrink-0">ซ่อมบำรุง</span>
                            </div>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">จ่ายจริง</div>
                            <div className="text-xs sm:text-sm font-semibold text-slate-700">฿{money(summary.main4Paid.repair)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-amber-700 leading-none mb-0.5">หัก ณ ที่จ่าย</div>
                            <div className="text-xs sm:text-sm font-semibold text-amber-700">฿{money(summary.main4Paid.repairDeduct)}</div>
                          </div>
                          <div className="h-6 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-emerald-800 leading-none mb-0.5">โอนจริง</div>
                            <div className="text-xs sm:text-sm font-extrabold text-emerald-700">฿{money(summary.main4Paid.repair - summary.main4Paid.repairDeduct)}</div>
                          </div>
                        </div>
                      </div>
                    );
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-2xs">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-[#0b3531] text-white font-bold border-b border-[#062e2b] text-[11px] uppercase tracking-wider">
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a]">รายการ</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">ยอดรวมค่าใช้จ่าย (จ่ายจริง)</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">หัก ณ ที่จ่าย (บาท)</th>
                        <th className="py-2.5 px-3 text-right">ยอดโอนสุทธิ (โอนจริง)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 bg-white font-normal text-slate-800">
                      <tr className="hover:bg-slate-50/80 transition">
            ...
            <div className="space-y-2.5">
              ...
              {viewMode === "cards" ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 sm:gap-2.5">
                  ...
                  {/* Card 1: เครื่องจักร */}
                  {(() => {
                    const paidNet = summary.main5PaidMachineTotal;
                    const pendingNet = summary.main5PendingMachineTotal;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          ...
                          <div className="min-w-0">
                            <span className="text-xs sm:text-sm font-bold text-slate-900 truncate block">เครื่องจักร</span>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                                  รอเบิก ฿{money(pendingNet)}
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.machineBeforeVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs font-semibold text-emerald-700">฿{money(summary.main5Paid.machineVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-500 leading-none mb-0.5">ไม่มี VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.machineNoVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                  ...
                  {/* Card 2: เครื่องมือ */}
                  {(() => {
                    const paidNet = summary.main5PaidToolTotal;
                    const pendingNet = summary.main5PendingToolTotal;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          ...
                          <div className="min-w-0">
                            <span className="text-xs sm:text-sm font-bold text-slate-900 truncate block">เครื่องมือ</span>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                                  รอเบิก ฿{money(pendingNet)}
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.toolBeforeVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs font-semibold text-emerald-700">฿{money(summary.main5Paid.toolVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-500 leading-none mb-0.5">ไม่มี VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.toolNoVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                  ...
                  {/* Card 3: อื่นๆ */}
                  {(() => {
                    const paidNet = summary.main5PaidOtherTotal;
                    const pendingNet = summary.main5PendingOtherTotal;
                    const hasData = paidNet > 0 || pendingNet > 0;
                    return (
                      <div className={`rounded-xl border p-2.5 sm:px-3 sm:py-2.5 flex items-center justify-between gap-2.5 transition-all ${hasData ? "bg-white border-slate-200/90 shadow-2xs hover:border-slate-300" : "bg-slate-50/50 border-slate-200/60 opacity-70"}`}>
                        <div className="flex items-center gap-2 min-w-0 shrink-0">
                          <div className="w-7 h-7 rounded-lg bg-purple-50 border border-purple-200/60 text-purple-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <Briefcase className="w-3.5 h-3.5" />
                          </div>
                          <div className="min-w-0">
                            <span className="text-xs sm:text-sm font-bold text-slate-900 truncate block">อื่นๆ</span>
                            <div className="text-[10px] sm:text-[11px] font-normal leading-tight mt-0.5">
                              {pendingNet > 0 ? (
                                <span className="font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/70 inline-block">
                                  รอเบิก ฿{money(pendingNet)}
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-auto text-right">
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-400 leading-none mb-0.5">ก่อน VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.otherBeforeVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-emerald-700 leading-none mb-0.5">VAT 7%</div>
                            <div className="text-xs font-semibold text-emerald-700">฿{money(summary.main5Paid.otherVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right">
                            <div className="text-[10px] font-medium text-slate-500 leading-none mb-0.5">ไม่มี VAT</div>
                            <div className="text-xs font-semibold text-slate-700">฿{money(summary.main5Paid.otherNoVat)}</div>
                          </div>
                          <div className="h-5 w-px bg-slate-200 shrink-0" />
                          <div className="text-right pl-0.5">
                            <div className="text-[10px] font-medium text-indigo-700 leading-none mb-0.5">รวมสุทธิ</div>
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-2xs">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-[#0b3531] text-white font-bold border-b border-[#062e2b] text-[11px] uppercase tracking-wider">
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a]">รายการ</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">เครื่องจักร (จ่ายจริง)</th>
                        <th className="py-2.5 px-3 border-r border-[#0d3f3a] text-right">เครื่องมือ (จ่ายจริง)</th>
                        <th className="py-2.5 px-3 text-right">อื่นๆ (จ่ายจริง)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 bg-white font-normal text-slate-800">
                      <tr className="hover:bg-slate-50/80 transition">
