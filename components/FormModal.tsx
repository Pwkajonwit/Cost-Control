...
export function FormModal({
  ...
  function populateFormValues(targetForm: FormPayload, detail?: OpenFormDetail) {
    ...
    const targetRowKey =
      detail?.sheetRow ??
      detail?.row?.id ??
      detail?.row?.["รหัสพนักงาน"] ??
      detail?.row?.["ลำดับ"] ??
      detail?.row?.["ID Project"] ??
      detail?.row?.id_people ??
      detail?.row?.id_store ??
      detail?.row?.id_Contractor ??
      detail?.row?.id_bank ??
      detail?.row?.id_car ??
      detail?.row?.id_cus ??
      detail?.row?.id_Company ??
      detail?.row?.id_Conwork ??
      detail?.row?._sheetRow;
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    if (submitValues["LINE User ID"] !== undefined) {
      submitValues["line_user_id"] = submitValues["LINE User ID"];
      submitValues["LINE"] = submitValues["LINE User ID"];
    }
