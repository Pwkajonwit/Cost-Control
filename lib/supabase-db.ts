...
export function mapSupabaseRowToSheetRow(dbTable: string, row: Record<string, any>, idx: number = 0): SheetRow {
  if (!row) return {};
  let dataObj = (row.data && typeof row.data === "object") ? { ...row.data } : {};
  while (dataObj.data && typeof dataObj.data === "object") {
    dataObj = { ...dataObj.data, ...dataObj };
    delete dataObj.data;
  }
  delete dataObj.data;
  const cleanRow = { ...row };
  delete cleanRow.data;
  const res: SheetRow = { ...dataObj, ...cleanRow };
  res.data = dataObj;
  ...
  if (dbTable === "bills") {
    ...
    const lineId = (row.line_user_id !== undefined && row.line_user_id !== null)
      ? String(row.line_user_id)
      : String(d.line_user_id ?? row["LINE User ID"] ?? d["LINE User ID"] ?? row["LINE"] ?? d["LINE"] ?? "");
    res["LINE"] = lineId;
    res["LINE User ID"] = lineId;
    res["line_user_id"] = lineId;
...
export function mapSheetRowToSupabaseRow(tableName: string, row: Record<string, any>): Record<string, any> {
  ...
  if (dbTable === "bills") {
    ...
    if (row["LINE User ID"] !== undefined || row["line_user_id"] !== undefined || row["LINE"] !== undefined || row["Line"] !== undefined || row["line"] !== undefined) {
      const lineVal = String(
        (row["LINE User ID"] !== undefined ? row["LINE User ID"] : undefined) ??
        (row["line_user_id"] !== undefined ? row["line_user_id"] : undefined) ??
        (row["LINE"] !== undefined ? row["LINE"] : undefined) ??
        (row["Line"] !== undefined ? row["Line"] : undefined) ??
    ...
    let rawData = (row.data && typeof row.data === "object") ? { ...row.data } : {};
    while (rawData.data && typeof rawData.data === "object") {
      rawData = { ...rawData.data, ...rawData };
      delete rawData.data;
    }
    delete rawData.data;
    delete rawData._sheetRow;
    const resolvedLineId = dbRow.line_user_id !== undefined ? dbRow.line_user_id : (row["LINE User ID"] ?? row["line_user_id"] ?? row["LINE"] ?? rawData["LINE User ID"] ?? rawData.line_user_id ?? rawData["LINE"] ?? "");
    dbRow.data = {
      ...rawData,
      ...
      "LINE User ID": resolvedLineId,
      "line_user_id": resolvedLineId,
      "LINE": resolvedLineId
    };
    delete dbRow.data.data;
    delete dbRow.data._sheetRow;
  } else if (dbTable === "contract_works") {
...
export async function updateRowInSupabase(tableName: string, keyColumn: string, keyValue: any, patch: Record<string, any>) {
  ...
  if (!isStatusOnly) {
    ...
    try {
      ...
      let existingData = (currentRecord && currentRecord.data && typeof currentRecord.data === "object") ? { ...currentRecord.data } : {};
      while (existingData.data && typeof existingData.data === "object") {
        existingData = { ...existingData.data, ...existingData };
        delete existingData.data;
      }
      delete existingData.data;
      delete existingData._sheetRow;

      let cleanPatch = { ...patch };
      while (cleanPatch.data && typeof cleanPatch.data === "object") {
        cleanPatch = { ...cleanPatch.data, ...cleanPatch };
        delete cleanPatch.data;
      }
      delete cleanPatch.data;
      delete cleanPatch._sheetRow;

      if (dbPatch.data && typeof dbPatch.data === "object") {
        let cleanDbPatchData = { ...dbPatch.data };
        delete cleanDbPatchData.data;
        delete cleanDbPatchData._sheetRow;
        dbPatch.data = { ...existingData, ...cleanPatch, ...cleanDbPatchData };
      } else {
        dbPatch.data = { ...existingData, ...cleanPatch };
      }
      if (dbPatch.line_user_id !== undefined) {
      ...
      delete dbPatch.data.data;
      delete dbPatch.data._sheetRow;
    } catch {
      let cleanPatch = { ...patch };
      delete cleanPatch.data;
      delete cleanPatch._sheetRow;
      dbPatch.data = cleanPatch;
      if (dbPatch.line_user_id !== undefined) {
        dbPatch.data.line_user_id = dbPatch.line_user_id;
        dbPatch.data["LINE User ID"] = dbPatch.line_user_id;
        dbPatch.data["LINE"] = dbPatch.line_user_id;
...
export async function insertRowToSupabase(tableName: string, rowData: Record<string, any>) {
  ...
  while (cleanRowData.data && typeof cleanRowData.data === "object") {
    cleanRowData = { ...cleanRowData.data, ...cleanRowData };
    delete cleanRowData.data;
  }
  delete cleanRowData.data;
  delete cleanRowData._sheetRow;
  if (dbRow.data && typeof dbRow.data === "object") {
    let cleanDbRowData = { ...dbRow.data };
    delete cleanDbRowData.data;
    delete cleanDbRowData._sheetRow;
    dbRow.data = { ...cleanRowData, ...cleanDbRowData };
  } else {
    dbRow.data = cleanRowData;
  }
  delete dbRow.data.data;
  delete dbRow.data._sheetRow;
